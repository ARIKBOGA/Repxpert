...
import { getSubfolderNamesSync, readJsonFile, retryListFilePath, padTrios } from "../utils/FileHelpers";
import { goToSearchResultsEnglish, ProductreferenceTrio, readProductReferencesTrio } from "../utils/ScraperHelpers";
...
test.describe("REPXPERT Aplikasyon bilgilerini al", () => {
  ...
  Array.from(padTrios)
    .filter(ref => {return ref.supplier.toUpperCase() === filterBrand.toUpperCase()})
    .forEach(ref => {
      ...
      crossNumbers.forEach(crossNumber => {
        ...
        test(`${filterBrand} - ${crossNumber} ürününün araç uyumluluklarını getir`, async ({ page }) => {
          ...
          try {
            ...
            const folderPath = path.join(`src/data/Gathered_Informations/${productType}/Applications/English`, productProducer || "UnknownBrand", crossNumber);
