...
const brand = process.env.FILTER_BRAND_APPLICATION as string;
...
test.describe('YV NO ve Marka bazlı teknik veri tarayıcı', async () => {
  ...
  for (const ref of padPairs) {
    ...
    const yvNo = ref.yvNo;
    let productCode = ref.brandRefs.ICER

    if (!productCode || !yvNo || !brand || productCode === "" || yvNo === "" || brand === "" || productCode === undefined || yvNo === undefined || brand === undefined
    ) {
      continue;
    }
    ...
    test(`${yvNo} / ${productCode} koduyla veri topla`, async ({ page }) => {
      ...
      try {
        ...
        const pad_attributes: Dimensions = {
        ...
        const product: Product = {
          ...
          eanNumber,
          pad_attributes, // change it accordingly
        };
